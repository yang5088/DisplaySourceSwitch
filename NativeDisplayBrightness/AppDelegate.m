...
//更改输入源 202009281921
- (NSString *)changeInputSourceKey {
    id changeInputSourceKeyCode = [NSUserDefaults.standardUserDefaults valueForKey:@"changeInputSourceKey"];
    if (!changeInputSourceKeyCode) {
        return @"F12";
    }
    return changeInputSourceKeyCode;
}
...
//更改输入源 202009281921
+(void)ChangeInputSource:(int) newInputSource {
    CGDirectDisplayID currentDisplayId = [NSScreen.mainScreen.deviceDescription [@"NSScreenNumber"] unsignedIntValue];
    if (! CGDisplayIsBuiltin(currentDisplayId)) {
        set_control(currentDisplayId, INPUT_SOURCE, newInputSource);
    }
}
